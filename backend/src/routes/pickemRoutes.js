...
router.get("/week", requireAuth, async (req, res) => {
  ...
  // számolt mezők (winner + canPick + correct)
  const now = new Date();
  ...
  const enriched = games.map(g => {
    ...
    let correct = null;
    if (final && picked) {
      correct = (winner !== "TIE") ? (picked === winner) : false; // döntetlen esetén jelenleg 0 pont
    }

    return {
      ...g,
      picked,
      canPick: !started,   // kickoff után tiltjuk
      started,
      final,
      winner,
      correct
  ...
  res.json({ season, week, games: enriched });
});
...
router.post("/pick", requireAuth, async (req, res) => {
  ...
  if (!validTeams.includes(String(picked).toUpperCase())) {
    return res.status(400).json({ error: "Érvénytelen csapat választás." });
  }

  const saved = await prisma.pickEmPick.upsert({
    where: { userId_gameId: { userId, gameId } },
    create: { userId, gameId, picked: String(picked).toUpperCase() },
    update: { picked: String(picked).toUpperCase() }
  });
...
async function recomputeWeekScore(season, week, userId) {
  ...
  const finalGames = games.filter(g => g.status === "FINAL" && g.homeScore != null && g.awayScore != null);
  ...
  const totalGames = games.length; // heti összes meccs
 ...
 * - visszaadja: heti pontok + összesített pontok (season total)
 */
router.get("/leaderboard", requireAuth, async (req, res) => {
  const season = Number(req.query.season || 2025);
  const week = Number(req.query.week || 1);

  // kiszámoljuk/cachelünk mindenkinek erre a hétre
  ...
  // season total = összes hét összege
 ...
 * - más user pickjei, de csak a már elkezdődött meccsek látszanak (anti-cheat)
 */
router.get("/user/:userId/picks", requireAuth, async (req, res) => {
  ...
  res.json({ season, week, userId: targetUserId, picks: result });
});
